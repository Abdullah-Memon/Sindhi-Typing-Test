...
const Result = ({ accuracy, wpm, wpmData, onRestart }) => {
  ...
  const allWpmValues = chartData.flatMap((d) =>
    [d.wpm, d.prevWpm].filter((val) => val !== null)
  );
  ...
  const maxTime =
    chartData.length > 0 ? Math.max(...chartData.map((d) => d.time)) : 0;
  const ticks = [];
  ...
  const avgWpm = Math.round(
    chartData.reduce((acc, d) => acc + d.wpm, 0) / chartData.length
  );

  return (
    <div className="mt-4 w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-5xl mx-auto text-center flex flex-col items-center h-full px-4">
      <h2 className="text-4xl sm:text-5xl font-bold text-[var(--text-primary-color)] mb-4">
        نتيجو
      </h2>
      <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 w-full justify-evenly border border-gray-100 p-4 shadow-md rounded mb-4">
        <p className="text-md sm:text-lg text-[var(--text-color)]">
          لفظن جي درستگي:{" "}
          <span className="text-[var(--text-primary-color)]">%{accuracy}</span>
        </p>
        <p className="text-md sm:text-lg text-[var(--text-color)]">
          لفظ في منٽ:{" "}
          <span className="text-[var(--text-primary-color)]">{wpm}</span>
        </p>
        <p className="text-md sm:text-lg text-[var(--text-color)]">
          لفظ في منٽ (AVG):{" "}
          <span className="text-[var(--text-primary-color)]">{avgWpm}</span>
        </p>
      </div>
      ...
      <div className="w-full" style={{ height: "300px" }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData}>
            ...
            <Tooltip
              contentStyle={{ backgroundColor: "#f6f6f6", borderRadius: 4 }}
            />
